...
import net.school.web.italker.push.bean.db.UserFollow;
...
public class UserFactory {
     ...
     * @return User
     */
    public static User register(String account, String password, String name, int type) {
        // 去除账户中的首位空格
        ...
        User user = createUser(account, password, name,type);
        if (user != null) {
            user = login(user);
        }
        return user;
     ...
     * @return 返回一个用户
     */
    private static User createUser(String account, String password, String name, int type) {
        User user = new User();
        user.setName(name);
